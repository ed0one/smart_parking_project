...
function Dashboard() {
    ...
    const [selectedParkingId, setSelectedParkingId] = useState(1); // Parcare implicită

    const { user, logout } = useAuth();
    const { toast } = useToast();
    const navigate = useNavigate();

    // Debug: Verifică starea curentă
    console.log("Dashboard mounted, current user:", user);
    console.log("AuthContext available:", !!useAuth);
    ...
    const handleLogout = () => {
        ...
        logout();
        console.log("After logout, navigating to /login");
        navigate("/login");
    };
    ...
    const handleParkingSelect = async (parkingId) => {
        setSelectedParkingId(parkingId);
        console.log("Selected parking ID:", parkingId);
        // Re-fetch data for the new parking
        setLoading(true);
        try {
            const data = await parkingApi.getParcareCompleta(parkingId);
            setParcareData(data);
            toast.success(`Parcare schimbată! Acum vizualizezi ${data.parcare.NUMEPARCARE}`);
        } catch (error) {
            console.error("Error switching parking:", error);
            toast.error("Eroare la schimbarea parcării");
        }
        setLoading(false);
    };
        ...
        <div className="App">
            <header className="App-header">
                <div className="header-container">
                    <h1><span className="parking-icon">🅿️</span> Parcare Inteligentă</h1>
                    <div className="user-info">
                        <span>Bun venit, {user?.NUME || 'Utilizator'}!</span>
                        <button onClick={handleAdminClick} className="admin-button" title="Acces la panoul de administrare">Admin</button>
                        <button onClick={handleLogout} className="logout-button" title="Ieșire din cont">Logout</button>
                    </div>
                </div>
            </header>
